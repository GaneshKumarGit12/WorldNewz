...
for asin in asins:
    ...
    try:
        with opener.open(url, timeout=12) as resp:
            ...
            for img in all_imgs:
                if any(x in img for x in ['PKmb-play', 'sprite', 'icon', 'logo', 'G/01', 'G/31', 'SX38', 'SY50']):
                    continue
                # Try clean SL1500
                ...
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=5) as t_resp:
                        if t_resp.status == 200:
                            results[asin] = cleaned
                            print(f"✅ Found working clean SL1500 image for {asin}: {cleaned}")
                            break
                except:
                    try:
                        ...
                        with urllib.request.urlopen(req, timeout=5) as t_resp:
                            if t_resp.status == 200:
                                results[asin] = img
                                print(f"✅ Found working original image for {asin}: {img}")
                                break
                    except:
                        pass
