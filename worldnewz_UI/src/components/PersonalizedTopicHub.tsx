...
export const PersonalizedTopicHub: React.FC<PersonalizedTopicHubProps> = ({
  ...
  const [internalActiveTopicId, setInternalActiveTopicId] = useState<string | null>(initialTopicId ?? null);
  const [articles, setArticles] = useState<Article[]>([]);
  const [loading, setLoading] = useState(false);
  const { addBookmark, removeBookmark, isBookmarked } = useBookmarks();
  const { followedTopicIds: hookFollowedTopicIds, toggleFollow: hookToggleFollow } = useFollowedTopics();

  const activeTopicId = propActiveTopicId !== undefined ? propActiveTopicId : internalActiveTopicId;

  const setActiveTopic = (id: string | null) => {
    setInternalActiveTopicId(id);
    if (onTopicSelect) {
      onTopicSelect(id);
    }
  };
  ...
  useEffect(() => {
    if (initialTopicId !== undefined) {
      setInternalActiveTopicId(initialTopicId);
    }
  }, [initialTopicId]);
  ...
  const activeTopic = useMemo(() => {
    ...
    return TOPIC_DEFINITIONS[activeTopicId] || null;
  }, [activeTopicId]);
  ...
  const isFollowed = activeTopic ? followedTopicIds.includes(activeTopic.id) : false;
  ...
  return (
    ...
    >
          ...
          {Object.values(TOPIC_DEFINITIONS).map((topic) => {
            ...
            return (
              <Chip
                ...
                label={topic.name}
                onClick={() => {
                  const next = isSelected ? null : topic.id;
                  setActiveTopic(next);
                }}
                sx={{
                  fontFamily: "var(--sans)",
      ...
      {/* 2. CONDITIONAL CONTENT: EMPTY STATE VS ACTIVE TOPIC */}
      {!activeTopic ? (
        <Box
          sx={{
            py: { xs: 5, sm: 6 },
        ...
        >
          ...
          >
            Select any topic from <strong>"Suggested for you"</strong> or click a category pill above to explore real-time editorial deep-dives, industry takeaways, verified FAQs, and live breaking streams.
          </Typography>
        </Box>
      ) : (
        <>
          {/* ACTIVE TOPIC HERO BANNER */}
          <Box
            sx={{
              p: { xs: 2.5, sm: 3.5 },
              mb: 4,
          ...
          {/* STRUCTURED 6 TO 10 PARAGRAPH EDITORIAL DEEP-DIVE */}
          <Box sx={{ mb: 5 }}>
